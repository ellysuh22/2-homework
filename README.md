...
메뉴에서 번호를 선택하면 퀴즈 풀기 · 추가 · 목록 · 삭제 · 점수 확인을 할 수 있는 콘솔 프로그램입니다.
외부 라이브러리 없이 파이썬 **표준 라이브러리**(`json`, `os`, `random`, `datetime`)만 사용했습니다.

이 미션이 확인하려는 4가지 학습 목표와, 이 프로젝트가 그것을 어떻게 만족하는지입니다.
...
| Git으로 변경 이력 관리 | 기능 단위 21커밋, 브랜치 생성 → 병합, GitHub 공개 |
...
기본 퀴즈 5개는 모두 직접 작성했으며, 코드에서는 [`default_quizzes()`](main.py#L61-L94)가
...
| 필수 요구 사항 | 상태 | 확인 위치 |
| --- | --- | --- |
| 메뉴 출력 + 기능 선택 + 종료 | ✅ 완료 | [`show_menu()`](main.py#L105-L117), [`run()`](main.py#L308-L326) |
| 퀴즈 풀기 (출제·정답 입력·정오답 안내·결과 표시) | ✅ 완료 | [`play_quiz()`](main.py#L248-L277), [`ask_one()`](main.py#L223-L246) |
| 퀴즈 추가 (문제·선택지 4개·정답 번호 입력 후 저장) | ✅ 완료 | [`add_quiz()`](main.py#L183-L195) |
| 퀴즈 목록 조회 | ✅ 완료 | [`list_quizzes()`](main.py#L197-L207) |
| 점수 확인 (최고 점수 비교·갱신·저장) | ✅ 완료 | [`show_score()`](main.py#L294-L306), [`record_result()`](main.py#L279-L292) |
| 퀴즈가 없는 경우 처리 | ✅ 완료 | [main.py:199-201](main.py#L199-L201), [main.py:250-252](main.py#L250-L252) |
| 아직 퀴즈를 풀지 않은 경우 처리 | ✅ 완료 | [main.py:296-298](main.py#L296-L298) |
| 클래스 2개 이상 정의 | ✅ 완료 | `Quiz`, `QuizGame` |
| 기능별 메서드 분리 | ✅ 완료 | 아래 [8번 항목](#8-입력-처리게임-진행저장을-분리한-기준) |
| 본인 주제 퀴즈 5개 이상 | ✅ 완료 | [`default_quizzes()`](main.py#L61-L94) — 5개 |
| 프로젝트 루트 `state.json`에 UTF-8로 저장/불러오기 | ✅ 완료 | [main.py:15](main.py#L15), [main.py:148](main.py#L148) |
| 재실행 시 퀴즈·최고 점수 유지 | ✅ 완료 | [`load_state()`](main.py#L119-L138) |
| 입력 검증 4종(공백/문자/범위 밖/빈 입력) | ✅ 완료 | [`input_int()`](main.py#L153-L172) |
| `Ctrl+C` / `EOF` 안전 종료 | ✅ 완료 | [`main()`](main.py#L329-L338) |
| 파일 없음 / 손상 시에도 실행 가능 | ✅ 완료 | [`load_state()`](main.py#L119-L138) |

### 보너스 기능 (선택 과제)

| 보너스 | 구현 | 위치 |
| --- | --- | --- |
| 랜덤 출제 | `random.sample()`로 매 판 순서를 섞음 | [main.py:258](main.py#L258) |
| 문제 수 선택 | 시작 전 몇 문제를 풀지 선택 | [main.py:256](main.py#L256) |
| 힌트 + 점수 차감 | 정답 입력창에서 `5` → 힌트, 1회당 5점 차감 | [main.py:228-238](main.py#L228-L238), [main.py:270](main.py#L270) |
| 퀴즈 삭제 | 번호로 삭제 후 파일 반영 | [`delete_quiz()`](main.py#L209-L221) |
| 점수 기록 히스토리 | 날짜/시간·푼 문제 수·정답 수·점수 저장 | [`record_result()`](main.py#L279-L292) |
...
| 2 | 퀴즈 추가 | 문제, 선택지 4개, 정답 번호를 입력받아 등록하고 파일에 저장합니다. |
...
숫자를 받는 모든 곳은 [`input_int()`](main.py#L153-L172) 하나를 거치므로, 아래 4가지가 **전 메뉴에서 동일하게** 처리됩니다.
...
문자를 받는 곳(문제·선택지 입력)은 [`input_text()`](main.py#L174-L181)가 빈 문자열을 막습니다.
...
- **`Quiz`** ([main.py:18-58](main.py#L18-L58)) — 퀴즈 한 문제
  - 속성: `question`, `choices`(4개), `answer`(1~4 번호), `hint`
  - 메서드: `display()`, `check()`, `to_dict()`, `from_dict()`
- **`QuizGame`** ([main.py:97-326](main.py#L97-L326)) — 게임 전체
...
**이렇게 나눈 이유 (역할과 책임 분리)** — "바뀌는 이유가 다르면 클래스도 다르다"는 기준입니다.
...
분리 기준은 **"바뀌는 이유가 다르면 함수도 다르다"** 입니다.

| 묶음 | 담당 메서드 | 왜 따로 뺐나 |
| --- | --- | --- |
| **입력 처리(검증)** | [`input_int()`](main.py#L153-L172), [`input_text()`](main.py#L174-L181) | 메뉴·정답·문제 수·삭제 번호 등 **6곳 이상에서 같은 검증**이 필요. 한곳에 모아야 규칙이 바뀔 때 한 번만 고침 |
| **게임 진행** | [`play_quiz()`](main.py#L248-L277), [`ask_one()`](main.py#L223-L246), [`record_result()`](main.py#L279-L292) | 출제 규칙·점수 규칙이 바뀔 때만 수정되는 부분 |
| **데이터 저장/불러오기** | [`load_state()`](main.py#L119-L138), [`save_state()`](main.py#L140-L151) | 저장 방식(JSON → DB)이 바뀌어도 **여기 2개만** 고치면 되도록 격리 |
...
| 한 판이 끝나 점수 기록 시 | `save_state()` | [main.py:291](main.py#L291) |
| `Ctrl+C` / `EOF` 발생 시 | 안내 후 `save_state()` | [main.py:335-338](main.py#L335-L338) |
...
`indent=2`로 사람이 읽기 좋게 들여썼습니다. ([main.py:148-149](main.py#L148-L149))

### `state.json` 스키마

> **스키마(schema)** = 그 파일에 "어떤 이름의 칸이 어떤 타입으로 들어가는지" 정한 약속.

- **경로**: 프로젝트 루트 `./state.json` (`main.py`와 같은 위치로 고정 — [main.py:15](main.py#L15))
...
1. `quizzes[]`의 필드 이름이 `Quiz` 클래스의 속성과 **1:1로 동일**합니다.
   그래서 [`to_dict()`](main.py#L41-L48) / [`from_dict()`](main.py#L50-L58)만으로 객체 ↔ 파일 변환이 끝나고,
...
`main()`에서 게임 루프 전체를 감쌌습니다. ([main.py:329-338](main.py#L329-L338))
...
`load_state()`에서 아래 4가지 실패를 한 번에 잡습니다. ([main.py:131](main.py#L131))
...
| `OSError` | 권한 없음, 디스크 문제 | 읽기 실패 시 복구 / 쓰기 실패 시 안내 ([main.py:150-151](main.py#L150-L151)) |
...
커밋 해시를 누르면 GitHub의 해당 커밋 변경 내용으로 바로 이동합니다.
전체 목록은 [GitHub 커밋 이력](https://github.com/ellysuh22/2-homework/commits/main)에서도 볼 수 있습니다.
...
> ⚠️ **증거 보완 필요** — `clone` / `pull` 명령을 실행한 **터미널 스크린샷**은 아직 `docs/`에
> 첨부하지 않았습니다. 커밋 이력만으로는 그 커밋이 clone된 디렉터리에서 만들어졌는지 구분되지 않으므로,
> 평가 전에 터미널 캡처를 `docs/screenshots/clone_pull.png`로 추가할 예정입니다.
...
| --- | --- | --- | --- | --- |
| **1-1** | 메뉴 및 4개 기능 동작 | [`run()`](main.py#L308-L326) · [`show_menu()`](main.py#L105-L117) | [기능 체크리스트](#4-필수-기능-체크리스트) · 커밋 [`07d8931`](https://github.com/ellysuh22/2-homework/commit/07d8931) | ✅ 완료 |
| **1-2** | 정오답 판정 + 입력 오류 4종 | [`input_int()`](main.py#L153-L172) · [`Quiz.check()`](main.py#L37-L39) | [잘못된 입력 처리](#잘못된-입력-처리--실제-출력-결과) · 커밋 [`a79f908`](https://github.com/ellysuh22/2-homework/commit/a79f908) | ✅ 완료 |
| **1-3** | 재실행 시 데이터 유지 | [`load_state()`](main.py#L119-L138) · [`save_state()`](main.py#L140-L151) | [저장 흐름](#9-실행부터-statejson-불러오기저장까지의-흐름) · 커밋 [`ff3840e`](https://github.com/ellysuh22/2-homework/commit/ff3840e) | ✅ 완료 |
| **1-4** | 기본 퀴즈 5개 이상 | [`default_quizzes()`](main.py#L61-L94) — 5개 | [주제 선정 이유](#2-퀴즈-주제와-선정-이유) · 커밋 [`8adca1c`](https://github.com/ellysuh22/2-homework/commit/8adca1c) | ✅ 완료 |
| **1-5** | GitHub 업로드 + 커밋 10개 이상 | — | [커밋 이력 17개](https://github.com/ellysuh22/2-homework/commits/main) · [README 커밋표](#전체-커밋-이력) | ✅ 완료 |
...
| **1-7** | clone·pull 실습 흔적 | — | 커밋 [`3bf2517`](https://github.com/ellysuh22/2-homework/commit/3bf2517) · [동기화 문구](#동기화-기록) | ⚠️ 증거 추가 필요 (터미널 스크린샷) |

### 항목 2 — 코드 구조 및 설계

| 평가번호 | 확인하려는 것 | 코드 증거 | README 설명 | 상태 |
| --- | --- | --- | --- | --- |
| **2-1** | 클래스 책임 분리 | [`Quiz`](main.py#L18-L58) · [`QuizGame`](main.py#L97-L326) | [7번 절](#7-quiz와-quizgame의-역할을-나눈-이유) | ✅ 완료 |
| **2-2** | 입력·진행·저장 분리 기준 | [`input_int()`](main.py#L153-L172) · [`play_quiz()`](main.py#L248-L277) · [`save_state()`](main.py#L140-L151) | [8번 절](#8-입력-처리게임-진행저장을-분리한-기준) | ✅ 완료 |
| **2-3** | `state.json` 읽기/쓰기 흐름 | 읽기 [332행](main.py#L332) / 쓰기 [194](main.py#L194)·[220](main.py#L220)·[291](main.py#L291)·[338행](main.py#L338) | [9번 절](#9-실행부터-statejson-불러오기저장까지의-흐름) | ✅ 완료 |
| **2-4** | 안전 종료 처리 | [`main()`](main.py#L329-L338) | [11번 절](#11-안전-종료와-파일-오류-처리) · 커밋 [`c961ad8`](https://github.com/ellysuh22/2-homework/commit/c961ad8) | ✅ 완료 |
| **2-5** | 커밋 단위·메시지 규칙 | — | [12번 절](#12-git-커밋브랜치병합clonepull-수행-내용) · [커밋 17개 전체](#전체-커밋-이력) | ✅ 완료 |

### 항목 3 — 핵심 기술 원리 적용

| 평가번호 | 확인하려는 것 | 코드 증거 | README 설명 | 상태 |
| --- | --- | --- | --- | --- |
| **3-1** | 클래스를 쓴 이유 | [`Quiz.__init__()`](main.py#L21-L25) · [`check()`](main.py#L37-L39) | [7번 절](#7-quiz와-quizgame의-역할을-나눈-이유) | ✅ 완료 |
| **3-2** | JSON 선택 이유·특징 | [`json.dump()` 148-149행](main.py#L148-L149) · [`json.load()` 127행](main.py#L127) | [10번 절](#10-json을-선택한-이유와-데이터-구조) | ✅ 완료 |
| **3-3** | `try/except`가 필요한 이유 | [읽기 예외 125-138행](main.py#L125-L138) · [쓰기 예외 147-151행](main.py#L147-L151) | [11번 절](#파일-오류-처리) | ✅ 완료 |
| **3-4** | 브랜치 분리 이유·merge 의미 | — | [브랜치와 병합](#브랜치와-병합) · [병합 커밋](https://github.com/ellysuh22/2-homework/commit/2d45928) | ✅ 완료 |
| **3-5** | `state.json` 구조 설계 이유 | [`to_dict()`](main.py#L41-L48) · [`from_dict()`](main.py#L50-L58) | [스키마 절](#statejson-스키마) | ✅ 완료 |

### 항목 4 — 심층 인터뷰

| 평가번호 | 확인하려는 것 | 코드 증거 | README 설명 | 상태 |
| --- | --- | --- | --- | --- |
| **4-1** | 퀴즈 1000개 시 한계 | [`save_state()` 전체 덮어쓰기](main.py#L140-L151) | [14-1](#14-1-퀴즈가-1000개-이상으로-늘어난다면-평가-4-1) | ✅ 완료 |
| **4-2** | 파일 손상 시 대응 | [손상 감지·복구 131-135행](main.py#L131-L135) | [14-2](#14-2-statejson이-손상되어-파싱에-실패한다면-평가-4-2) | ✅ 완료 |
| **4-3** | 요구사항 변경 시 수정 위치 | [`HINT_PENALTY`](main.py#L12) · [점수식 270행](main.py#L270) | [14-3](#14-3-채점-방식이나-퀴즈-구조가-바뀐다면-평가-4-3) | ✅ 완료 |
...
**현재 구현** — [`load_state()`](main.py#L119-L138)는 파일 전체를 한 번에 읽어 메모리에 올리고,
[`save_state()`](main.py#L140-L151)는 변경이 생길 때마다 **전체를 통째로 다시 씁니다.**
...
| --- | --- | --- |
| 점수 계산 변경 (예: 난이도별 배점) | [`play_quiz()`](main.py#L248-L277) 270행 점수 계산식, [`record_result()`](main.py#L279-L292) | `Quiz` 클래스 전체, 저장 로직 |
| 힌트 감점 변경 | 상수 `HINT_PENALTY` ([main.py:12](main.py#L12)) 한 줄 | 나머지 전부 |
| 선택지 4개 → 5개 | `Quiz.choices` 검증·[`display()`](main.py#L27-L35), [`add_quiz()`](main.py#L183-L195)의 입력 루프, `ask_one()`의 입력 범위, 힌트 번호 상수 `HINT_CHOICE`, 그리고 `state.json` 스키마 문서 | `load_state()`/`save_state()`의 저장 로직 |
